...
const getMorningFormData = async (request) => {
  ...
  if (request) {
    ...
    morningFormData.genericMoodMorning = parseInt(
      params.get("generic_mood_morning")
    );
    if (isNaN(morningFormData.sleepDuration))
      morningFormData.sleepDuration = null;
  }
...
const getEveningFormData = async (request) => {
  ...
  if (request) {
    ...
    eveningFormData.exercisesDuration = parseFloat(
      params.get("exercises_duration")
    );
    ...
    eveningFormData.genericMoodEvening = parseInt(
      params.get("generic_mood_evening")
    );
    if (isNaN(eveningFormData.exercisesDuration))
      eveningFormData.exercisesDuration = null;
    if (isNaN(eveningFormData.studyDuration))
      eveningFormData.studyDuration = null;
  }

  return eveningFormData;
};

const showReportingPage = async ({ render, state, cookies }) => {
  let user = await state.session.get("user");
  const dateValue = getDate();
  const errors = (await cookies.get("errors"))
    ? JSON.parse(await cookies.get("errors"))
    : {};
  let morningFormData = await state.session.get("morningFormData");
  morningFormData = morningFormData ? morningFormData : {};
  let eveningFormData = await state.session.get("eveningFormData");
  ...
  render("behaviour/reporting.ejs", {
    ...
    user,
    reporting: await cookies.get("reporting"),
    errors,
    dateToday: dateValue,
    data: Object.assign(morningFormData, eveningFormData),
    morningReported: await reportingService.isReportedForMorning(
      user.user_id,
      dateValue
    ),
    eveningReported: await reportingService.isReportedForEvening(
      user.user_id,
      dateValue
    ),
  });
  await cookies.set("reporting", null);
  await cookies.set("errors", null);
  await state.session.set("morningFormData", null);
  await state.session.set("eveningFormData", null);
};

const postMorningReport = async ({ request, response, state, cookies }) => {
  const user = await state.session.get("user");
  const data = await getMorningFormData(request);
  await state.session.set("morningFormData", data);
  const [passes, errors] = await validate(data, morningValidationRules, {
    messages: morningValidationMessages,
  });
  if (!passes) {
    await cookies.set("errors", JSON.stringify(errors));
    await cookies.set("reporting", "morning");
...
const postEveningReport = async ({ request, response, state, cookies }) => {
  const user = await state.session.get("user");
  const data = await getEveningFormData(request);
  await state.session.set("eveningFormData", data);
  const [passes, errors] = await validate(data, eveningValidationRules, {
    messages: eveningValidationMessages,
  });
  if (!passes) {
    await cookies.set("errors", JSON.stringify(errors));
    await cookies.set("reporting", "evening");
    response.redirect("/behaviour/reporting");
  } else {
